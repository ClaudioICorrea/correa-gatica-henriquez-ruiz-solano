...
for nk in range(nkmax):
    ...
    Vh = FunctionSpace(mesh, MixedElement([H,H,H,Q,X,M,X,M,X,M,R_]))
    dof.append(Vh.dim())
    ...
    #H  ,H   ,H,    Q,  X,  M,   X,   M,   X,   M,R
    taux,tauy,tauz, v,psi,lam,tau1,eta1,tau2,eta2,zeta  = TestFunctions(Vh)
    sigx,sigy,sigz, u,phi,chi,sig1,xi1,sig2,xi2  ,theta = split(sol)
    ...
    sigx_h, sigy_h, sigz_h, u_h, phi_h, chi_h, sig1_h, xi1_h, sig2_h, xi2_h  ,theta_h = sol.split()
    ...
    mass1_ = project(xi1_h - div(sig1_h) - f1,Ph)
    mass2_ = project(xi2_h - div(sig2_h) - f2,Ph)
    energy_ = project(div(phi_h) + (xi1_h-xi2_h) + f,Ph)
    
    PPh = VectorFunctionSpace(mesh,'DG',k)
    mom_ = project(ff- (xi1_h - xi2_h)/epsilon*phi_h + div(sig_h), PPh)
